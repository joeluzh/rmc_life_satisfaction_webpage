...
author: "Joël" # Remember to add your name!
date: "16-10-2024"
theme: united
---
...
cantril_table <- cantril %>%
  ...
  mutate(`Relative change` = (`2023` / `2011` - 1) * 100) %>%
  mutate(`Relative change` = ifelse(`Relative change` < 0, 
                                    paste0("-", percent(abs(`Relative change`)/100, accuracy = 0.01)), 
                                    paste0(percent(`Relative change`/100, accuracy = 0.01)))) %>% 
...
cantril_plot_table <- cantril %>% 
  select(Country, Year, `Cantril ladder score`) %>% 
  filter(Country %in% c("Angola", "Switzerland", "Finland", "Niger", "China", "Russia", "Iran", "Nicaragua", "Canada", "Iceland"))

plot <- ggplot(cantril_plot_table, aes(x = Year, y = `Cantril ladder score`, colour = Country)) +
  geom_line() +
  theme_minimal() +
  scale_x_continuous(breaks = seq(2011,2023,1)) +
  labs(title = "Self-reported life satisfaction, 2011 to 2023", y = NULL, x = NULL) +
